...
class IBag(ABC, Generic[T]): #ABC is a parent class, generic will allow to create any bag 
    """
    Interface for a Bag (MultiSet) data structure.
    """
                                                                        #Iterable implies it can/cant
    @abstractmethod                                                     #annotation/decorator in Python, expected to not return, not providing defn "pass"
    def __init__(self, *items: Optional[Iterable[T]]) -> None:          #*items allows to pass in mult parameters
        """                                                                    
        Initializes the Bag.
        """
        pass
